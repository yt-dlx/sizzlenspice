...
export async function GET(request: NextRequest) {
  ...
  const orders = userEmail
    ? await prisma.order.findMany({ where: { userId: userEmail } })
...
export async function PUT(request: NextRequest) {
  ...
  return NextResponse.json(
    { message: "Order status updated successfully", result },
    { status: 200 }
  );
}
...
export async function DELETE(request: NextRequest) {
  ...
  return result
    ? NextResponse.json({ message: "Order cancelled successfully", result }, { status: 200 })
