...
export default function HomePage() {
  ...
  const Indicator = ({ isVeg }: { isVeg: boolean }) => (
    <div className={`w-4 h-4 rounded-full ${isVeg ? "bg-lime-400" : "bg-red-600"}`} />
  );
        ...
        >
          ...
          >
            <Image
              ...
              height={1080}
              alt={item.title}
              className="object-cover w-full h-72 rounded-t-3xl mb-4"
            />
            <div className="px-2">
              ...
              <div className="flex justify-center items-center text-center flex-wrap gap-2">
                {Object.entries(item.price).map(([size, price]) => (
                  ...
                  >
                    <FaRupeeSign /> {size.charAt(0).toUpperCase() + size.slice(1)}{" "}
                    <MdAlternateEmail /> {price}
                  </button>
                ))}
              </div>
              <button
                onClick={onClose}
                className="w-full bg-[#E9F0CD] text-[#172B25] py-2 mb-2 rounded-3xl mt-4 hover:bg-[#8C9A68] transition duration-300 font-Kurale font-bold"
              >
                Cancel Item
              </button>
            </div>
  ...
  useEffect(() => {
    ...
    const categoryItems =
      activeCategory === "All"
        ? allItems
        : categories.find((cat: any) => cat.title === activeCategory)?.items || [];
  ...
  return (
    <main className="max-w-full mx-auto overflow-hidden bg-gradient-to-b from-[#1C3029]/30 from-10% via-[#171717] via-40% to-[#111111] to-50% p-4">
      {isModalOpen && selectedItem && (
        <SizeSelectionModal item={selectedItem} onClose={() => setIsModalOpen(false)} />
      )}
      ...
      >
        <div className="flex flex-col gap-1 w-full">
          <div className="relative w-full">
            <FaSearch
              size={20}
              className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#172B25]"
            />
            <input
              type="text"
              value={searchTerm}
      ...
      {cart.length > 0 && (
        <section id="cart-button" className="fixed bottom-14 right-2">
          <button
            ...
            className="bg-[#E9F0CD] text-[#172B25] p-2 rounded-lg shadow shadow-[#111111] flex items-center"
          >
