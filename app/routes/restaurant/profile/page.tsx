      ...
      <section id="categories" className="max-w-2xl sm:max-w-4xl md:max-w-6xl lg:max-w-7xl flex items-center justify-center mx-auto py-2">
        <div className="flex scrollbar-thin scrollbar-thumb-secondary scrollbar-track-primary overflow-x-auto space-x-2 pb-4">
          {restaurantData?.categories?.map((category, index) => (
            <button
              key={index}
              onClick={() => setEditingCategory(category.title)}
              className={`flex flex-col items-center shadow-md shadow-secondary/20 p-1 rounded-xl w-24 text-primary ${editingCategory === category.title ? "bg-secondary/90" : "bg-secondary/20 text-secondary"}`}
            >
          ...
          <div className="p-4 w-full scrollbar-thin scrollbar-thumb-primary scrollbar-track-secondary overflow-x-auto">
            ...
            <div className="flex items-center mb-4">
              <div className="flex flex-col">
                <Image src={selectedItem?.image || ""} alt={selectedItem?.title || "Item image"} width={400} height={300} className="rounded-full object-cover w-36 h-36 border-2 border-secondary" />
            ...
            <button
              onClick={() => handleUpdateItem(editingItem!.categoryTitle, editingItem!.itemIndex)}
            ...
            >
              <FaSave className="text-secondary" /> Confirm & Update
            </button>
          </div>
        </section>
      )}
