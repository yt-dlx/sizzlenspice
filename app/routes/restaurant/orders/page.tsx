        ...
        setOrderTimers((prevTimers) => {
          ...
          orders.forEach((order) => {
            if (order.status !== "Completed")
              newTimers[order._id] = (newTimers[order._id] || 0) + 1;
          });
          return newTimers;
        });
      }, 1000);
    ...
    return (
      ...
      >
        <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl text-secondary">
          <TypeAnimation
            sequence={["Order Management", 2000]}
            repeat={Infinity}
            wrapper="span"
            speed={2}
          />
        ...
        <h2 className="text-lg sm:text-2xl md:text-3xl py-2">
          Manage and track all incoming orders efficiently in one place, all in real-time!
        </h2>
      </motion.section>
    );
  };
  const DashboardSummary = () => {
    return (
      <section id="DashboardSummary">
        <header className="text-4xl mb-1 bg-secondary text-primary shadow shadow-[#131313] px-3 py-2 rounded-lg flex items-center justify-center">
          My Work Summary
        </header>
        <ul className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <li className="bg-secondary/10 p-1 rounded-lg shadow shadow-[#131313] border-2 border-[#131313] text-secondary flex flex-col items-center justify-center text-center">
            ...
            <article className="text-3xl md:text-4xl font-semibold text-center">
              {orders.filter((order) => order.status === "Pending").length}
            </article>
          </li>
          <li className="bg-secondary/10 p-1 rounded-lg shadow shadow-[#131313] border-2 border-[#131313] text-secondary flex flex-col items-center justify-center text-center">
            <header className="text-xl">Preparing</header>
            <article className="text-3xl md:text-4xl font-semibold">
        ...
        {selectedOrder ? (
          <div className="bg-secondary/10 rounded-lg shadow-md p-4 text-secondary">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => toggleVisualize(selectedOrder._id)}
                className="bg-secondary text-primary px-3 py-1 rounded-full flex items-center text-xs"
              >
            ...
            <div className="mb-4">
              <ul className="list-disc m-4">
                <li className="text-sm">
                  <span className="font-bold text-lg mr-2">User:</span>{" "}
                  {selectedOrder.userId.split("@")[0]}
                </li>
                <li className="text-sm">
                ...
                <li className="text-sm">
                  <span className="font-bold text-lg mr-2">Date & Time:</span>{" "}
                ...
                <li className="text-sm">
                  <span className="font-bold text-lg mr-2">Address:</span>{" "}
                  {selectedOrder.locationData?.address}
                </li>
                <li className="text-sm">
                  <span className="font-bold text-lg mr-2">Pincode:</span>{" "}
                  {selectedOrder.locationData?.pincode}
                </li>
                <li className="text-sm">
                ...
                {selectedOrder.items.map((item, index) => (
                  <div key={index} className="flex items-center bg-secondary/10 p-4 rounded-lg">
                    <Image
                      ...
                      alt={item.title}
                      src={item.image}
                      className="object-cover w-14 h-14 rounded-full shadow shadow-[#131313] border-2 border-[#131313] mr-4"
                    />
              ...
              <div className="flex flex-wrap justify-center sm:justify-end space-x-2">
                {statusOptions.map((status) => (
                  <button
                    key={status}
                    onClick={() =>
                      updateOrderStatus(selectedOrder._id, status, selectedOrder.userId)
                    }
                    className={`p-2 mb-2 transition duration-700 ease-in-out transform rounded-full ${
                      selectedOrder.status === status
                        ? "bg-[#A8B67C] text-primary"
                        : "bg-secondary hover:bg-[#A8B67C] text-primary"
                    } flex items-center`}
                  >
          ...
          <p className="p-4 rounded-lg bg-secondary/10 text-secondary">
        ...
        <div className="lg:col-span-1">
          <div className="bg-secondary/10 rounded-lg shadow-md p-4 text-secondary">
            ...
            <motion.input
              ...
              className="w-full p-2 mb-4 rounded-md bg-[#1C2924] text-secondary"
            />
            <div className="space-y-4">
              {orders.map((order) => (
                <div
                  key={order._id}
                  className={`p-4 rounded-lg cursor-pointer ${selectedOrder?._id === order._id ? "bg-[#1C2924]" : "bg-secondary/10"}`}
                  ...
                  <div className="flex justify-between items-center">
                    <div className="font-bold">
                      <ul className="list-disc ml-4">
                        <li>
                          Status:{" "}
                          <span className="text-semibold animate-pulse">#{order.status}</span>
                        </li>
                        <li>
